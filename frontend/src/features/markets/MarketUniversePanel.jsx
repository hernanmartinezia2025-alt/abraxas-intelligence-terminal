...
export default function MarketUniversePanel({ selected, onChange, showCorrelations = true }) {
  ...
  return (
    <section className="market-universe-panel exchange-panel">
      ...
      {overview && <section className={`macro-context-strip ${overview.regime}`}>
        ...
        {showCorrelations && <>
          <div className="macro-correlation-head"><span>CORRELACIONES · PEARSON SOBRE RETORNOS DIARIOS</span><small>Binance/SQLite + FRED · minimo {overview.correlation_method?.minimum_samples || 20} muestras</small></div>
          {overview.correlation_reading && <div className={`btc-correlation-reading ${overview.correlation_reading.strength || "insufficient"}`}><div><span>BTC CROSS-ASSET READ</span><strong>{overview.correlation_reading.dominant_pair || "SIN MUESTRA"} {overview.correlation_reading.correlation === undefined ? "" : Number(overview.correlation_reading.correlation).toFixed(2)}</strong></div><p>{overview.correlation_reading.summary}</p><small>{overview.correlation_reading.caveat}</small></div>}
          <div className="macro-correlation-row">{(overview.correlations || []).map((pair) => <article className={pair.status === "ready" ? pair.relationship : "insufficient"} key={`${pair.left}-${pair.right}`}><span>{pair.left} / {pair.right}</span><strong>{pair.correlation === null ? "N/A" : Number(pair.correlation).toFixed(2)}</strong><small>{pair.status === "ready" ? `${pair.relationship} · ${pair.samples} retornos` : `datos insuficientes · ${pair.samples}`}</small></article>)}</div>
        </>}
      </section>}
