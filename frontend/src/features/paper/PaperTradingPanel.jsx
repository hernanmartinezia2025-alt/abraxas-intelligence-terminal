  ...
  return <>
    ...
    <section className="exchange-panel paper-orders">
      ...
      <div className="execution-intent-audit">
        ...
        <div className="backtest-trades-wrap"><table><thead><tr><th>Intent</th><th>Adapter</th><th>Symbol</th><th>Action</th><th>Qty</th><th>Status</th><th>Result</th></tr></thead><tbody>{(snapshot.execution_intents || []).map((intent) => <tr key={intent.id}><td title={intent.id}><code>{intent.id.slice(0, 8)}</code></td><td>{intent.adapter}</td><td>{intent.symbol}</td><td>{intent.action}</td><td>{intent.quantity}</td><td className={intent.status === "filled" ? "positive" : intent.status === "rejected" || intent.status === "failed" ? "negative" : ""}>{intent.status}</td><td>{intent.result_reference || "pending"}</td></tr>)}</tbody></table>{!(snapshot.execution_intents || []).length && <div className="chart-state">Sin intenciones de ejecucion en esta sesion paper.</div>}</div>
      </div>
